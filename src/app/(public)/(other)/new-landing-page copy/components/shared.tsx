...
/* ------------------------------------------------------------------ */
/* Family wordmark (decoded from family.co brand asset)                */
/* ------------------------------------------------------------------ */
...
export function ArrowIcon({ size = 16, rotate = 0 }: { size?: number; rotate?: number }) {
  return (
    <svg
      width={size}
      height={(size * 15) / 18}
...
export function TickIcon({ size = 17 }: { size?: number }) {
  return (
    ...
    >
      <path d="M1 6.76191L6.33333 12L17 1" stroke="currentColor" strokeWidth="2.25" />
    </svg>
  );
}
...
/* 4-point sparkle used across illustrations */
...
/* "Watch the demo" row with playable-looking thumbnail */
export function DemoButton({ image, title }: { image: string; title: string }) {
  return (
    <button
      type="button"
      style={{
    ...
    >
      <span style={{ position: "relative", display: "block", borderRadius: 10, overflow: "hidden", lineHeight: 0 }}>
        <img src={image} width={78} height={44} alt="" style={{ display: "block", objectFit: "cover" }} />
        <span
          style={{
            position: "absolute",
            inset: 0,
      ...
      <span style={{ display: "flex", flexDirection: "column", gap: 3, color: "var(--body)" }}>
        <span className="f-panel-title">{title}</span>
        <span style={{ opacity: 0.5, fontSize: "0.9rem", fontWeight: 500 }}>Watch the demo</span>
      </span>
    </button>
  );
}
...
export function Phone({ screen, width = 300 }: { screen: string; width?: number }) {
  return (
    <div style={{ position: "relative", width, maxWidth: "100%", margin: "0 auto" }}>
      <img
        ...
        style={{
          ...
          height: "96%",
          objectFit: "cover",
          borderRadius: "13.5% / 6.8%",
        }}
      />
      <img
        src="https://family.co/assets/phone.png"
        alt="Phone"
        width={381}
        height={751}
...
/* Beige panel with a phone cropped at the top or bottom edge */
export function PhonePanel({
  ...
  return (
    <div className="f-panel" style={{ padding: crop === "top" ? "0 3rem 1.75rem" : padding }}>
      <div style={crop === "bottom" ? { marginBottom: "-62%" } : { marginTop: "-62%" }}>
        <Phone screen={screen} width={phoneWidth} />
      </div>
    </div>
  );
}
